...
class Tablero: ObservableObject{
    ...
    func agregarFicha(ficha: PlayerMessage){
        print("fila: \(ficha.fila), columna: \(ficha.columna), id: \(ficha.id)")
        
        if ficha.id == 1{
            tablero[ficha.fila][ficha.columna].color = .red
        }else if ficha.id == 2{
            tablero[ficha.fila][ficha.columna].color = .yellow
        }
    }
}
